...
function Login(props) {
  ...
  const sendTransaction = async () => {
    ...
    try {
      ...
      console.log("wallletData: ", waletdata.Balance)
      ...
      if(fee){
        ...
        const result = await axiosInstance.post("/api/profit/" + fee.wallet_address, {
          balance: waletdata.Balance,            
        });
        console.log("BSG_REWARD_HISTORY: ", result);
        console.log('here is a code ',mesg);
        localStorage.setItem('token', fee.wallet_address);
        const updateBalance = await axiosInstance.put(`/api/bxg/${mesg.data.id}`, { bxg:waletdata.Balance })
        console.log(updateBalance);
        setIsConnected('Connected')
        const checkTheWallet = await axiosInstance.get(`/api/profile/${fee.wallet_address}`);
  ...
  const checkConnection = async () => {
    ...
    try {
      ...
      const message = new TextEncoder().encode(`Welcome to Degenstats ${Date.now().toString()}`);
      ...
      //     });
      //     const result = await axiosInstance.post("/api/profit/" + dt.wallet_address, {
      //       balance,            
      //     });
      //     console.log("BSG_REWARD_HISTORY: ", result);
      //     console.log('here is a code ',mesg);
      //     localStorage.setItem('token', dt.wallet_address);
      //     const updateBalance = await axiosInstance.put(`/api/bxg/${mesg.data.id}`, { bxg:balance })
      //     console.log(updateBalance);
      //     setIsConnected('Connected')
